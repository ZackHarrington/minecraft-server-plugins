...
public class Manager {
	...
	public static void createNewWorld(Player player, boolean addDimensions, boolean tmp, String name) {
	...
	private static void createNewNetherWorld(Player player, World world, Boolean tmp) {		
		WorldCreator wc = new WorldCreator(world.getName() + "_nether");
		wc.environment(Environment.NETHER);
		World newWorld = wc.createWorld();
		if (tmp) {
			worldNethers.put(world, newWorld);
		}
	}
	
	private static void createNewEndWorld(Player player, World world, Boolean tmp) {
		WorldCreator wc = new WorldCreator(world.getName() + "_the_end");
		wc.environment(Environment.THE_END);
		World newWorld = wc.createWorld();
		if (tmp) {
			worldEnds.put(world, newWorld);
		}
	}
