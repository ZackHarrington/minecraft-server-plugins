	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (sender instanceof Player) {
			...
			if (args.length == 1 && args[0].equalsIgnoreCase("?")) {
				if (Manager.isInApocalypse(player.getWorld())) {
					player.sendMessage(ChatColor.YELLOW + "This world is in the midst of a zombie apocalypse");
				} else {
					player.sendMessage(ChatColor.YELLOW + "There is no zombie apocalypse in this world. yet....");
				}
				...
				if (args[1].equalsIgnoreCase("true") || args[1].equalsIgnoreCase("t")) {
					...
					player.sendMessage(ChatColor.RED + " - /zombieapocalypse end <kill zombies>");
					player.sendMessage(ChatColor.RED + " - /zombieapocalypse ?");
				}
